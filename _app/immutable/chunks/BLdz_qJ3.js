...
concrete — see ./rust-patterns#dependency-injection.
...
await, or switch to \`tokio::sync::*\`. See ./rust-perf §Async lock hygiene.
...
See ./wasm-patterns for build targets, WIT design, and optimization
profiles.
...
\`parking_lot\` remains the default for sync sections (./rust-patterns
...
- **Take \`&str\` / \`&[T]\`, not \`String\` / \`Vec<T>\`**, unless you need
  ownership. Already covered in ./rust-patterns §Avoid clone smells —
...
- **Cross-crate inlining**: \`#[inline]\` only inlines within a crate by
  ...
  (./rust-patterns §Release Profile), which enables cross-crate
...
- **Crate-specific SIMD features**: some crates expose \`simd\` cargo
  ...
  uses \`wasm32_simd\` for WASM SIMD (./wasm-patterns).
...
(\`blake3_component\`, N-API bindings; see ./rust-patterns §Lints).
...
| Rust patterns (WASM errors)      | ./rust-patterns                                                              |
| Rust performance                 | ./rust-perf                                                                  |
